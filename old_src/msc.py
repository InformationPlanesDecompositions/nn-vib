...
def plot_x_y(
    ...
    if point_labels:
        for x, y in zip(xs, ys1):
            ax.text(x, y, f"({x:.4f}, {y:.2f})", fontsize=8, verticalalignment="bottom", horizontalalignment="right")

            if ys2:
                for x, y in zip(xs, ys2):
                    ax.text(
                        x, y, f"({x:.4f}, {y:.2f})", fontsize=8, verticalalignment="top", horizontalalignment="left"
                    )
...
def plot_information_plane(ces: List[float], kls: List[float], save_dir: str):
    ...
    for i in range(num_epochs - 1):
        ...
        plt.plot(i_x_t[i : i + 2], i_t_y[i : i + 2], color=color, linestyle="-", linewidth=2, alpha=0.7, zorder=1)

    scatter = plt.scatter(i_x_t, i_t_y, c=epochs, cmap=cmap_name, norm=norm, marker="o", s=50, alpha=1.0, zorder=2)
...
def plot_losses(test_losses: List[float], train_losses: List[float], file_name: str, save_dir: str) -> None:
    ...
    plt.plot(
        range(1, epochs + 1), test_losses, marker="o", linewidth=2, markersize=6, color="#ff7f0e", label="test Loss"
    )
