...
def plot_page(
    ...
    save_name = (
        f"mlp_pruning_{prune_method}_{layer_key(list(method_key))}{page_suffix}_acc.pdf"
    )
...
def plot_aggregate_page(
    ...
    save_name = (
        f"mlp_pruning_{prune_method}_{layer_key(list(method_key))}_aggregate_acc.pdf"
    )
    save_path = os.path.join(output_dir, save_name)
    fig.savefig(save_path)
    plt.close(fig)
...
def print_max_robustness_table(
    ...
    for config_key, beta_samples in config_items:
        ...
        for beta, curves in filtered_beta_items(beta_samples):
            ...
            print(
                f"{layer_key(list(method_key))},{hidden1},{hidden2},{z_dim},"
                f"{float(lr):g},{epochs},{beta:g} -> {mean[max_idx]:.8g}, "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="plot aggregated mlp pruning robustness from a json report"
    )
    parser.add_argument(
        "--input_json",
        type=str,
        required=True,
        help="json report produced by inspect_mlp_ib.py",
    )
    parser.add_argument(
        "--plots_dir",
        type=str,
