...
if __name__ == "__main__":
  ...
  for run_name in sorted(os.listdir(save_root)):
    ...
    hidden1_s, hidden2_s, decoder_hidden_s, z_dim_s, beta_s, lr_s, epochs_s, seed_s = match.groups()
    if float(beta_s) != 0.0:
      continue
    ...
    key = (int(hidden1_s), int(hidden2_s), int(decoder_hidden_s), int(z_dim_s), float(lr_s), int(epochs_s))
    grouped[key].append((int(seed_s), float(stats["test_accs"][-1])))

  print("hidden1,hidden2,decoder_hidden,z_dim,lr,epochs,num_seeds,avg_final_test_acc,seeds")
  for key in sorted(grouped):
    ...
    print(f"{hidden1},{hidden2},{decoder_hidden},{z_dim},{lr:g},{epochs},{len(values)},{avg_acc:.6f},{seeds}")
