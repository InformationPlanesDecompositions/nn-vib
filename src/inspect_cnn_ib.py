...
def outgoing_prune_layers(model: VIBNet, layer_names: list[str], amount: float) -> None:
    ...
    for layer_name in layer_names:
        ...
        if next_layer_spec is None:
            raise ValueError(
                f"layer has no outgoing layer to prune against: {layer_name}"
            )
        ...
        if edge_type == "linear":
            ...
            if layer_name != "fc_mu_logvar" or not isinstance(next_layer, nn.Linear):
                raise ValueError(
                    f"unsupported latent-linear mapping: {layer_name} -> {next_layer_name}"
                )
            scores = next_layer.weight.detach().abs().mean(dim=0)
        else:
...
def prune_layers(
    model: VIBNet, layer_names: list[str], amount: float, prune_method: str
) -> None:
...
def parse_all_run_specs(root_dir: str) -> list[RunSpec]:
    ...
    for run_name in os.listdir(root_dir):
        ...
        h1_s, h2_s, decoder_hidden_s, z_s, beta_s, lr_s, epochs_s, seed_s = (
            match.groups()
        )
        runs.append(
            RunSpec(
...
def group_runs_by_config(
    runs: list[RunSpec],
) -> list[tuple[tuple[int, int, int, int, int, float, int], list[RunSpec]]]:
    grouped = {}
    for run in runs:
        key = config_key(run)
...
def build_report(save_root: str, data_dir: str, prune_method: str) -> dict[str, object]:
    ...
    test_loader = DataLoader(
        CIFAR10Dataset(
            data_dir, train=False, transform=CIFAR10Dataset.test_transform(data_dir)
        ),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="inspect cnn pruning stability across an entire save root"
    )
    parser.add_argument(
        "--save_root",
        type=str,
    ...
    parser.add_argument(
        "--data_dir", type=str, default="data/CIFAR-10/", help="cifar-10 dataset path"
    )
    parser.add_argument(
        "--prune_method",
...
if __name__ == "__main__":
    ...
    if not os.path.isdir(args.data_dir):
        raise RuntimeError(
            f"data_dir does not exist or is not a directory: {args.data_dir}"
        )

    report = build_report(args.save_root, args.data_dir, args.prune_method)
    json_path = os.path.join(
        args.save_root, f"cnn_pruning_report_{args.prune_method}.json"
    )
