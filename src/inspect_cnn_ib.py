...
def indices_above_avg_abs_threshold(
    model: nn.Module, layer_name: str, top_k: int, axis: str = "row"
) -> dict[int, float]:
...
device = get_device()
test_loader = DataLoader(CIFAR10Dataset("data/CIFAR-10/", train=False), batch_size=100, shuffle=False)
...
def mask_keep_neurons(
    weight_matrix: torch.Tensor, keep_row_indices: list[int] | None = None, keep_col_indices: list[int] | None = None
...
with torch.no_grad():
    for layer_name, keep_rows in to_prune_neurons.items():
        ...
        pruned_weights, _ = mask_keep_neurons(layer.weight, keep_row_indices=keep_rows)
        layer.weight.copy_(pruned_weights)

pruned_loss, _, _, pruned_acc = evaluate_epoch(dict_pruned_model.to(device), test_loader, device, beta=beta)
