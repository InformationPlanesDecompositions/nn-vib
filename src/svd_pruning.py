...
def get_pruning_threshold(s_vals, energy_threshold: float):
    ...
    if len(indices) == 0: return len(s_vals)
    k = indices[0].item() + 1
    return k
...
for idx, (layer_name, thresh) in enumerate(layer_names_w_thresh):
    ...
    if low_rank_params >= original_params: continue
    ...
    print(f"layer {layer_name}: pruned rank from {len(S)} to {k} (saved {savings:,} params, {100*savings/original_params:.1f}%)")
...
axes = axes.flatten()
for i in range(num_layers, len(axes)): axes[i].axis('off')
