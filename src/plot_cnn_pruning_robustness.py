...
def accuracy_robustness_curve(
    reference_values: list[float], values: list[float]
) -> np.ndarray:
    eps = 1e-8
...
def add_beta_colorbar(
    fig, axes, beta_min: float, beta_max: float
) -> tuple[object, object]:
    cmap = plt.get_cmap(beta_colormap)
    norm = SymLogNorm(linthresh=1e-5, vmin=beta_min, vmax=beta_max)
    scalar_map = plt.cm.ScalarMappable(norm=norm, cmap=cmap)
    colorbar = fig.colorbar(scalar_map, ax=axes, ticks=[beta_min, beta_max])
...
def build_samples(
    ...
    panel_samples: dict[
        tuple[str, ...], dict[tuple[object, ...], dict[float, list[np.ndarray]]]
        ...
        for layer_result in config["layer_results"]:
            ...
            beta_zero_curve = next(
                (
                    ...
                    if math.isclose(float(curve["beta"]), beta_zero)
                ),
                None,
            )
            ...
            for curve in layer_result["curves"]:
                ...
                if existing is None:
                    ...
                    raise ValueError(
                        f"inconsistent prune percents for method {method_key}"
                    )

                normalized = accuracy_robustness_curve(
                    beta_zero_curve["accuracies"], curve["accuracies"]
                )
...
def plot_page(
    ...
    if beta_labels == "colorbar":
        ...
        cmap, norm = add_beta_colorbar(
            fig, axes_list[: len(config_items)], beta_min, beta_max
        )
    ...
    save_name = (
        f"cnn_pruning_{prune_method}_{layer_key(list(method_key))}{page_suffix}_acc.pdf"
    )
...
def plot_aggregate_page(
    ...
    save_name = (
        f"cnn_pruning_{prune_method}_{layer_key(list(method_key))}_aggregate_acc.pdf"
    )
    save_path = os.path.join(output_dir, save_name)
    fig.savefig(save_path)
    plt.close(fig)
...
def print_max_robustness_table(
    method_key: tuple[str, ...],
    config_items: list[tuple[tuple[object, ...], dict[float, list[np.ndarray]]]],
    prune_percents: list[float],
) -> None:
    print("table_values_begin")
    print(
        "layers,hidden1,hidden2,decoder_hidden,z_dim,lr,epochs,beta,max_robustness,pruning_fraction"
    )
    for config_key, beta_samples in config_items:
        hidden1, hidden2, decoder_hidden, z_dim, lr, epochs = config_key
        for beta, curves in filtered_beta_items(beta_samples):
            mean, _ = summarize_curves(curves)
            max_idx = int(np.argmax(mean))
            print(
                f"{layer_key(list(method_key))},{hidden1},{hidden2},{decoder_hidden},"
                f"{z_dim},{float(lr):g},{epochs},{beta:g},{mean[max_idx]:.8g},"
                f"{prune_percents[max_idx]:g}"
            )
    print("table_values_end")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="plot aggregated cnn pruning robustness from a json report"
    )
    parser.add_argument(
        "--input_json",
        type=str,
        required=True,
        help="json report produced by inspect_cnn_ib.py",
    )
    parser.add_argument(
        "--plots_dir",
        type=str,
    ...
    parser.add_argument(
        "--beta_labels",
        choices=beta_label_choices,
        default="colorbar",
        help="label beta lines with a table or colorbar",
    )
...
if __name__ == "__main__":
    ...
    for method_key in sorted(panel_samples):
        sorted_config_items = sorted(
            panel_samples[method_key].items(), key=lambda item: item[0]
        )
        print_max_robustness_table(
            method_key,
            sorted_config_items,
