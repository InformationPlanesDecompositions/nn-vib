...
def plot_page(
  ...
  save_name = f"cnn_pruning_{prune_method}_{layer_key(list(method_key))}{page_suffix}_acc.pdf"
...
def plot_aggregate_page(
  ...
  save_name = f"cnn_pruning_{prune_method}_{layer_key(list(method_key))}_aggregate_acc.pdf"
  save_path = os.path.join(output_dir, save_name)
  fig.savefig(save_path)
  plt.close(fig)
  return save_path
